...
void main() {
  ...
  // D-04/D-05: readXSyncedAt() is null before any write for that key, and a
  // recent DateTime immediately after the paired writeX() call — one test
  // per remaining key pair (Task 2).

  test(
    'readHomepageSyncedAt is null before any write and recent after '
    'writeHomepage()',
    () async {
      final cache = CacheService.instance;

      expect(await cache.readHomepageSyncedAt(), isNull);

      await cache.writeHomepage({'username': 'alice', 'bandsCount': 3});

      final syncedAt = await cache.readHomepageSyncedAt();
      expect(syncedAt, isNotNull);
      expect(DateTime.now().difference(syncedAt!).inSeconds, lessThan(5));
    },
  );

  test(
    'readBandsSyncedAt is null before any write and recent after '
    'writeBands()',
    () async {
      final cache = CacheService.instance;

      expect(await cache.readBandsSyncedAt(), isNull);

      await cache.writeBands([
        {'id': 'b1', 'name': 'The Testers'},
      ]);

      final syncedAt = await cache.readBandsSyncedAt();
      expect(syncedAt, isNotNull);
      expect(DateTime.now().difference(syncedAt!).inSeconds, lessThan(5));
    },
  );
  ...
  test(
    'readBandDetailSyncedAt is null before any write and recent after '
    'writeBandDetail()',
    () async {
      final cache = CacheService.instance;

      expect(await cache.readBandDetailSyncedAt('b1'), isNull);

      await cache.writeBandDetail('b1', {'id': 'b1', 'name': 'The Testers'});

      final syncedAt = await cache.readBandDetailSyncedAt('b1');
      expect(syncedAt, isNotNull);
      expect(DateTime.now().difference(syncedAt!).inSeconds, lessThan(5));
    },
  );
  ...
  test(
    'readBandTracksSyncedAt is null before any write and recent after '
    'writeBandTracks()',
    () async {
      final cache = CacheService.instance;

      expect(await cache.readBandTracksSyncedAt('b1'), isNull);

      await cache.writeBandTracks('b1', [
        {'id': 't1', 'title': 'Song One', 'artist': 'Artist One'},
      ]);

      final syncedAt = await cache.readBandTracksSyncedAt('b1');
      expect(syncedAt, isNotNull);
      expect(DateTime.now().difference(syncedAt!).inSeconds, lessThan(5));
    },
  );
  ...
  test(
    'readBandTrackDetailSyncedAt is null before any write and recent after '
    'writeBandTrackDetail()',
    () async {
      final cache = CacheService.instance;

      expect(await cache.readBandTrackDetailSyncedAt('b1', 't1'), isNull);
      ...
      final syncedAt = await cache.readBandTrackDetailSyncedAt('b1', 't1');
      expect(syncedAt, isNotNull);
      expect(DateTime.now().difference(syncedAt!).inSeconds, lessThan(5));
    },
  );
  ...
  test(
    'readUserTracksSyncedAt is null before any write and recent after '
    'writeUserTracks()',
    () async {
      final cache = CacheService.instance;

      expect(await cache.readUserTracksSyncedAt(null), isNull);
      ...
      final syncedAt = await cache.readUserTracksSyncedAt(null);
      expect(syncedAt, isNotNull);
      expect(DateTime.now().difference(syncedAt!).inSeconds, lessThan(5));
    },
  );
  ...
  test(
    'readBandSetlistsSyncedAt is null before any write and recent after '
    'writeBandSetlists()',
    () async {
      final cache = CacheService.instance;

      expect(await cache.readBandSetlistsSyncedAt('b1'), isNull);
      ...
      final syncedAt = await cache.readBandSetlistsSyncedAt('b1');
      expect(syncedAt, isNotNull);
      expect(DateTime.now().difference(syncedAt!).inSeconds, lessThan(5));
    },
  );
  ...
  test(
    'readSetlistDetailSyncedAt is null before any write and recent after '
    'writeSetlistDetail()',
    () async {
      final cache = CacheService.instance;

      expect(await cache.readSetlistDetailSyncedAt('b1', 's1'), isNull);
      ...
      final syncedAt = await cache.readSetlistDetailSyncedAt('b1', 's1');
      expect(syncedAt, isNotNull);
      expect(DateTime.now().difference(syncedAt!).inSeconds, lessThan(5));
    },
  );
  ...
  test(
    'readUserSetlistsSyncedAt is null before any write and recent after '
    'writeUserSetlists()',
    () async {
      final cache = CacheService.instance;

      expect(await cache.readUserSetlistsSyncedAt(null), isNull);
      ...
      final syncedAt = await cache.readUserSetlistsSyncedAt(null);
      expect(syncedAt, isNotNull);
      expect(DateTime.now().difference(syncedAt!).inSeconds, lessThan(5));
    },
  );
